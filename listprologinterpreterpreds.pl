interpretpart(is,Variable1,Value1,Vars1,Vars2) :-
        getvalue(Variable1,Value1A,Vars1),
	isvalstr(Value1),
        val1emptyorvalsequal(Value1A,Value1),
	%%isval(Value2),
        putvalue(Variable1,Value1,Vars1,Vars2),
		(debug(on)->(writeln([call,[variable,is,Value1],"Press c."]),(not(get_single_char(97))->true;abort));true),
		(debug(on)->(writeln([exit,[Value1,is,Value1],"Press c."]),(not(get_single_char(97))->true;abort));true).
