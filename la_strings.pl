...
timeout(N2,A,N,S) :-
	...
	(catch(call_with_time_limit(Time_limit,(writeln_info(["Trying",A]),catch(A,_,fail))),time_limit_exceeded,fail)->writeln_info([A,"successful"]);(N1 is N-1,(N1=0->true;%true%
	sleep(S)%)
	%)
